...
from backup.model_19 import Sg2ImModel

from preprocessing.delaunay_triangulation_glands import vertex, delaunay_triangulation, draw_point

parser = argparse.ArgumentParser()
parser.add_argument('--checkpoint', default='./BestModelsYet/GraphToMask/checkpoint_with_model.pt')
...
parser.add_argument('--use_loc_feature', default=0, type=int)

args = parser.parse_args()


#VOCAB = {"benign": 0, "lowgrade": 1, "highgrade": 2}
...
def json_to_img(scene_graph, model):
    ...
    with torch.no_grad():
        ...
        image_pred, trimask_pred, object_masks_pred, object_boxes = model(object_indices=None,
                                                                    object_coordinates=None,
                                                                    object_embeddings=object_embeddings,
                                                                    triples=triples,
                                                                    objects_boxes_gt=objects_boxes_gt)

    # Save the component mask layout
    for i in range(trimask_pred.shape[0]):
        mask_path = os.path.join('images', output_dir, 'mask{}.png'.format(current_time))
        save_image(trimask_pred[i], mask_path)
        #return_img_layout_path = os.path.join('images', output_dir, 'img_layout{}.png'.format(current_time))
        #return_img_layout_path = img_layout_path
        img_path = os.path.join('images', output_dir, 'image{}.png'.format(current_time))
        save_image(image_pred[i], img_path)

    img_layout_path = os.path.join('images', output_dir, 'graph{}.png'.format(current_time))
    #save_image(img_sample, img_path)
    matplotlib.image.imsave(img_layout_path, img_sample)
