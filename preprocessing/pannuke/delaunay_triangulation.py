...
def draw_point(img, p, color):
    ...
    #cv2.putText(img, str(p[0]) + ',' + str(p[1]), (p[0], p[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.6, label_color, 2)
...
def retrieve_vertex(vertices, pt_id):
    for v in vertices:
        if (v.object_id == pt_id):
            return v
    raise "Error in retrieve_vertex function in delaunay_triangulation_glands.py"
...
def get_nuclei_type(color_mask, point, bounding_box):
    ...
    if (np.array_equal(color_mask[point[1]][point[0]], [0,0,0])):
        return "Neoplastic"
    if (np.array_equal(color_mask[point[1]][point[0]], [0,128,0])):
        return "Inflammatory"
    if (np.array_equal(color_mask[point[1]][point[0]], [0,255,0])):
        return "Inflammatory"
    if (np.array_equal(color_mask[point[1]][point[0]],[255,255,0])):
        return "Soft"
    if (np.array_equal(color_mask[point[1]][point[0]],[255,0,0])):
        return "Dead"
    if (np.array_equal(color_mask[point[1]][point[0]], [0,0,255])):
        return "Epithelial"
    # print(point)
    # print(color_mask[point[1]][point[0]])
    # print("Error in retriving nuclei type")
